...
export default function Page() {
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <SiteHeader />
        <main className="flex flex-col gap-12 py-12 px-4 md:px-8 max-w-5xl w-full mx-auto bg-gradient-to-br from-blue-50 via-white to-pink-50 rounded-3xl shadow-xl">
          {/* Hero Image */}
          ...
          <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <Card className="w-full px-4 py-6 sm:p-6 bg-white/80 backdrop-blur-md border-0 shadow-lg">
              <CardContent>
                <h2 className="text-2xl font-bold text-primary">{t("dove_siamo")}</h2>
                <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">📍 {t("indirizzo")}</p>
                <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">📞 {t("telefono")}: <a href="tel:+393318571453" className="text-blue-600 hover:underline">{t("telefono_numero")}</a></p>
                <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">📧 {t("email")}: <a href="mailto:info@rodyna.it" className="text-blue-600 hover:underline">{t("email_valore")}</a></p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <Card className="w-full px-4 py-6 sm:p-6 bg-gradient-to-br from-primaryBlue/10 to-pink-100 border-0 shadow-2xl">
              <CardContent>
                ...
                <form ref={form} onSubmit={handleSend} className="space-y-6">
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1">
                      <label htmlFor="name" className="block text-sm font-medium">{t("nome")}</label>
                      <input type="text" id="name" name="user_name" className="mt-1 block w-full border rounded-lg px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring focus:border-blue-500" required />
                    </div>
                    <div className="flex-1">
                      <label htmlFor="email" className="block text-sm font-medium">{t("email")}</label>
                      <input type="email" id="email" name="user_email" className="mt-1 block w-full border rounded-lg px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring focus:border-blue-500" required />
                    </div>
                  </div>
                  <div>
                    <label htmlFor="message" className="block text-sm font-medium">{t("messaggio")}</label>
                    <textarea id="message" name="message" rows={4} className="mt-1 block w-full border rounded-lg px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring focus:border-blue-500" required></textarea>
                  </div>
                  <button type="submit" className="bg-primaryBlue text-white font-semibold px-8 py-3 rounded-lg hover:bg-blue-700 transition disabled:opacity-60 text-lg shadow-md" disabled={loading}>
                    {loading ? t("invio_in_corso") : t("invia")}
                  </button>
                  {success && (<p className="text-green-600 font-medium mt-2">{t("messaggio_inviato_successo")}</p>)}
            ...
            <Card className="w-full px-4 py-6 sm:p-6 bg-white/80 backdrop-blur-md border-0 shadow-lg">
              <CardContent>
                ...
                <p className="text-sm text-blue-600 mt-2">
                  <a href="https://www.google.com/maps?ll=40.904552,14.395128&z=19&t=m&hl=it&gl=IT&mapclient=embed&cid=5086467851719980605" target="_blank" rel="noopener noreferrer">
                    {t("apri_google_maps")}
                  </a>
                </p>
              </CardContent>
